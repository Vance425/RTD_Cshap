...
namespace RTDDAC
{
    ...
    public class DBPool : IDisposable
    {
        ...
        /// 
        /// </summary>
        public bool CheckConnet(out string msg)
        {
            try
            {
                msg = "";
        ...
        public bool DisConnet(out string msg)
        {
            try
            {
                msg = "";
        ...
        internal bool SQLExec(List<string> sqlList, out string errSql, bool commit)
        {
            ...
            {
                //this.KeepTrans = false;
                tmpMsg = string.Format("[{0}][{1}][{2}][{3}]", "DBAccess", "Exception", _position, e.Message);
                if (logger is not null)
                {
                    if (!errSql.Equals(""))
